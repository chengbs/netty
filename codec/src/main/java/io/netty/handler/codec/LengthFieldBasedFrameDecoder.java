...
public class LengthFieldBasedFrameDecoder extends ByteToMessageDecoder {
    ...
    private final int maxFrameLength; // 包的最大长度
    private final int lengthFieldOffset; // 长度域的偏移量
    private final int lengthFieldLength; // 长度域长度
    private final int lengthFieldEndOffset; // 紧跟长度域字段后面的第一个字节的在整个数据包中的偏移量
    private final int lengthAdjustment; // 包体长度调整的大小，长度域的数值表示的长度加上这个修正值表
    private final int initialBytesToStrip; // 跳过某些字节
    private final boolean failFast; // 如果为true，则表示读取到长度域，TA的值的超过maxFrameLength，就抛出一个 TooLongFrameException，而为false表示只有当真正读取完长度域的值表示的字节之后，才会抛出 TooLongFrameException，默认情况下设置为true，建议不要修改，否则可能会造成内存溢出
    private boolean discardingTooLongFrame; // 丢弃模式
    private long tooLongFrameLength; // 数据包长度超出最大包长度的长度
    private long bytesToDiscard; // 表示还需要丢弃多少字节
